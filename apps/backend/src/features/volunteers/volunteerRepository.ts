...
export async function updateVolunteer(id: string, input: UpdateVolunteerInput): Promise<VolunteerRecord> { const current = await getVolunteer(id); const updated = { ...current, ...input, id: current.id, updatedAt: new Date().toISOString() }; await dynamoDb.send(new PutCommand({ TableName: tableName, Item: updated, ConditionExpression: "attribute_exists(id)" })); return updated; }
